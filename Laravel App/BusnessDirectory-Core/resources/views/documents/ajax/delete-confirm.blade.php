<div class="col-md-12">
 ...
 <thead>
	...
	<!-- <th>{{App\Global_var::getLangString('Uploaded_By', $language_strings)}}</th>
	<th>{{App\Global_var::getLangString('Updated_By', $language_strings)}}</th>
 --> </thead>
<tbody>
<tr>
	<td><h4>{{$document->title}}</h4></td>
	<td><h4>{{$document->category}}</h4></td>
	<td><h4>{{$document->uploadedDateTime}}</h4></td>
	<!-- <td>{{$document->createdByUser!=null? $document->createdByUser->username():''}}</td>
