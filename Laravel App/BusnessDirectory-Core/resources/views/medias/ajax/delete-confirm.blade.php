...
<hr>
	...
	<label class="label label-default">{{$department->createdByUser!=null? $department->createdByUser->name:''}}</label>

	{{App\Global_var::getLangString('Updated_By', $language_strings)}}:
	<label class="label label-default">{{$department->updatedByUser!=null? $department->updatedByUser->name:''}}	</label>
	<br>
