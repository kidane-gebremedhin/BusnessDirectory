...
class BackupController extends Controller
{
    ...
    {   
        $connect = new PDO('mysql:host=localhost;dbname=justice_office','root','root');
        if ($connect == false) {
            dd('Arafankis');
        }
        ...
        foreach ($result as $key => $table) {
            // dd($table['Tables_in_justice_office']);
            $show_query_table = "SHOW CREATE TABLE " . $table['Tables_in_justice_office'] . "";
            ...
            $select_query = "SELECT * FROM " . $table['Tables_in_justice_office'] . "";
            ...
            for ($count=0; $count < $total_row; $count++) { 
                ...
                $tt = $table['Tables_in_justice_office'];
                $output .= "\nINSERT INTO $tt (";
